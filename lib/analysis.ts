import { ExtractedSkills, SkillCategory, SKILL_CATEGORIES, getActiveCategories, hasExtractedSkills } from "./skills";
...
// Recalculate readiness score based on skill confidence
...
export function analyzeJD(
  ...
  const generalFresherStack = !hasExtractedSkills(extractedSkills);

  // If no skills detected, show general fresher stack
  if (generalFresherStack) {
    // Add default skills for general fresher
    extractedSkills.coreCS = ["DSA", "OOP", "DBMS"];
    extractedSkills.languages = ["Java/Python/C++"];
    extractedSkills.web = ["Basic Web"];
  }
